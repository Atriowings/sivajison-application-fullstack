      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-3 md:py-4 border-b border-gray-200">
          ...
          <div className="hidden md:flex space-x-6 lg:space-x-8 items-center">
            <a href="https://www.sivajison.com/"  target='_blank' className="text-sm md:text-base text-gray-600 hover:text-green-600 transition-colors font-medium">Home</a>
            <a href="https://www.sivajison.com/about-us/" target='_blank' className="text-sm md:text-base text-gray-600 hover:text-green-600 transition-colors font-medium">About Us</a>
            <a href="https://www.sivajison.com/contact-us/" target='_blank' className="text-sm md:text-base text-gray-600 hover:text-green-600 transition-colors font-medium">Contact Us</a>
           <a href="https://payments.cashfree.com/forms/sivajison" target='_blank'><button 
              className="px-4 md:px-6 py-2 md:py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center font-medium text-sm md:text-base animate-bounce" 
        ...
        {mobileMenuOpen && (
          <div className="md:hidden py-2 space-y-2">
            <a href="https://www.sivajison.com/"  target='_blank' className="block px-3 py-2 text-sm rounded-md font-medium text-gray-700 hover:text-green-600 hover:bg-gray-50">Home</a>
            <a href="https://www.sivajison.com/about-us/" target='_blank' className="block px-3 py-2 text-sm rounded-md font-medium text-gray-700 hover:text-green-600 hover:bg-gray-50">About Us</a>
            <a href="https://www.sivajison.com/contact-us/" target='_blank' className="block px-3 py-2 text-sm rounded-md font-medium text-gray-700 hover:text-green-600 hover:bg-gray-50">Contact Us</a>
...
const CustomerBookingPage = () => {
  ...
  const [searchType, setSearchType] = useState('phone'); // 'phone' or 'pnr'
  ...
  const validateInput = () => {
    if (searchType === 'phone') {
      if (!searchInput.match(/^[\+]?[(]?[0-9]{1,4}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{3,7}$/)) {
  ...
  const fetchBookingDetails = async () => {
    ...
    try {
      let hotelResponse, flightResponse;
      
      if (searchType === 'phone') {
        [hotelResponse, flightResponse] = await Promise.all([
          axios.get(`${API_BASE_URL}/customer/hotel/${searchInput}`),
          axios.get(`${API_BASE_URL}/customer/flight/${searchInput}`)
        ]);
      } else { // PNR search
        [hotelResponse, flightResponse] = await Promise.all([
          axios.get(`${API_BASE_URL}/customer/hotel/${searchInput}`),
      ...
      if (flightResponse.data && flightResponse.data.length > 0) {
        ...
        // Fetch logos for all unique airlines
        const airlines = new Set();
        flightResponse.data.forEach(booking => {
          if (booking.airlines && booking.airlines.length > 0) {
            booking.airlines.forEach(airline => {
              if (airline.airline) {
        ...
        const logoPromises = Array.from(airlines).map(async airline => {
          const logo = await fetchAirlineLogo(airline);
          return { airline, logo };
        });

        const logos = await Promise.all(logoPromises);
        const logoMap = logos.reduce((acc, { airline, logo }) => {
          acc[airline] = logo;
          return acc;
        }, {});
            ...
            <div className="p-4 sm:p-6 md:p-8">
              <div className="text-center mb-6 md:mb-8">
                ...
                <p className="text-sm sm:text-base text-gray-600 max-w-2xl mx-auto">
                  Find and manage your flight and hotel bookings by entering your phone number or PNR
                </p>
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-4 md:p-6 mb-6 md:mb-8 border border-gray-200">
                ...
                <div className="flex mb-3">
                  <button
                    ...
                    className={`px-4 py-2 rounded-l-lg font-medium text-sm ${searchType === 'phone' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                  >
                    Phone Number
                  </button>
                  <button
                    onClick={() => setSearchType('pnr')}
                    className={`px-4 py-2 rounded-r-lg font-medium text-sm ${searchType === 'pnr' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                  >
                    PNR Number
                  </button>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-3 md:gap-4">
                  <div className="relative flex-grow">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      {searchType === 'phone' ? <FiPhone className="text-gray-400" /> : <FiHash className="text-gray-400" />}
                    </div>
                    <input
                      ...
                      maxLength={searchType === 'phone' ? '15' : '20'}
                    />
                  </div>
                  <button
                          ...
                          {flightBookings.map((booking, index) => (
                            <div key={index} className="border-b border-gray-200 pb-4 md:pb-6 mb-4 md:mb-6 last:border-b-0 last:pb-0 last:mb-0">
                              <div className="flex justify-between items-start mb-4">
                                <div>
                                  <h3 className="text-md font-semibold text-gray-800">
                                    Booking #{index + 1} - {booking.airlines?.[0]?.airline }
                                  </h3>
                                  {/* <p className="text-sm text-gray-500">
                                    {formatDate(booking.departureDateTime)} to {formatDate(booking.arrivalDateTime)}
                                  </p> */}
                                </div>
                                <FlightPdfDownloadButton 
                                  booking={booking}
                                  logo={logo}
                                  iata={iata}
                                  airlineLogo={airlineLogos[booking.airlines?.[0]?.airline]}
                                  formatTime={formatTime}
                                  formatDateWithDay={formatDateWithDay}
                                  formatPassengerNames={formatPassengerNames}
                                />
...
const HotelPdfDownloadButton = ({ booking, logo, iata, formatDateWithDay, formatTime, calculateNights, formatPassengerNames }) => {
  const { toPDF, targetRef } = usePDF({
    filename: `hotel-booking-${booking.bookingReference}.pdf`,
    page: {
      margin: 20,
      format: 'A4',
  ...
  return (
    <>
      <button
        ...
        className="flex items-center px-3 md:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium text-sm md:text-base"
      >
...
const FlightPdfDownloadButton = ({ booking, logo, iata, airlineLogo, formatTime, formatDateWithDay, formatPassengerNames }) => {
  const { toPDF, targetRef } = usePDF({
    filename: `flight-ticket-${booking.bookingReference }.pdf`,
    page: {
      margin: 20,
      format: 'A4',
      orientation: 'portrait'
...
const HotelBookingDetails = ({ bookingDetails, formatDate, formatDateWithDay, calculateNights, formatPassengerNames}) => {
  return (
    <>
...
const HotelPdfTemplate = ({ bookingDetails, logo, iata, formatDateWithDay, formatTime, calculateNights, formatPassengerNames }) => {
  return (
    <div style={{
      width: '210mm',
      minHeight: '297mm',
        ...
        <div style={{ }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <tbody>
              ...
              {/* new line added */}
               <tr>
                <td style={{ padding: '8px 0', borderBottom: '1px solid #eee', width: '40%', fontWeight: '600' }}>Additional Guest :</td>
                <td style={{ padding: '8px 0', borderBottom: '1px solid #eee', fontWeight: '500' }}>{formatPassengerNames(bookingDetails.passengername)}</td>
              </tr>
              <tr>
              ...
              <thead>
                <tr style={{ 
                  ...
                  <th style={{ padding: '10px', textAlign: 'left' }}>Arrival</th>
                   <th style={{ padding: '10px', textAlign: 'left' }}>Terminal</th>
