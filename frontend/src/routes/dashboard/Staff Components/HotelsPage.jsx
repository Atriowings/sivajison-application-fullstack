...
// add passenger newly adding to here
   const addPassenger = () => {
      setFormData(prev => ({
        ...prev,
...
// here code ends
  ...
  const handleChange = (e) => {
    ...
    // When check-in date changes, clear checkout date if it's now before check-in
    if (name === "checkindate" && value && formData.checkoutdate && formData.checkoutdate < value) {
      setFormData(prev => ({
        ...prev,
        checkoutdate: ""
      }));
  ...
  // Date validation - now allows checkout date to be same as check-in
    ...
    if (confirmed) {
      ...
      try {
        ...
        const bookingData = {
          ...
          hcn: formData.hcn,
          // newly added
          passengername: formData.passengerNames.map(passengername => ({ passengername })),
          hotelcontactperson: formData.hotelcontactperson
          
        };

         // console.log(response.data)("Sending data to backend:", bookingData);
        ...
        const response = await axios.post(
         ...
         // console.log(response.data)("API Response:", response.data);
...
<div className="space-y-1">
  ...
  <div className="relative">
    <input
      ...
      onChange={handleChange}
      min={formData.checkindate}  // Allow same day as check-in
      className={`w-full px-4 py-3 rounded-lg border focus:ring-2 focus:outline-none transition-all ${
        errors.checkoutdate 
          ? "border-red-500 focus:ring-red-300 dark:focus:ring-red-700" 
          : "border-gray-300 dark:border-gray-600 focus:border-green-600 focus:ring-green-200 dark:focus:ring-green-800"
...
<div className=" space-y-2">
  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
    Additional Guest <span className="text-red-500">*</span>
  </label>
                ...
                >
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                          <span className="text-green-600">Additional Guest:</span> {formatPassengerNames(booking.passengerNames)}
                        </p>
                      <div>
                        <p className="text-sm text-gray-600 dark:text-gray-400">
