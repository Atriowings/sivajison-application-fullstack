...
const FlightsPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    airlines: [{
      ...
      departureAirport: "",
      arrivalAirport: "",
      departureDate: "",  // New separate date field
      departureTime: "",  // Time only
      arrivalDate: "",    // New separate date field
      arrivalTime: "",    // Time only
      travelClass: "Economy",
      baggageAllowance: "",
      terminal: ""
    }]
...
useEffect(() => {
  ...
  const handleAirlineChange = (index, e) => {
    ...
    setFormData(prev => {
      ...
      // If departure date changes, clear arrival date if it's now invalid
      if (name === 'departureDate' && value && newAirlines[index].arrivalDate < value) {
        newAirlines[index].arrivalDate = "";
      }
  ...
  // }
    ...
    formData.airlines.forEach((airline, index) => {
      ...
      if (!airline.departureDate) newErrors[`departureDate-${index}`] = "Departure date is required";
      if (!airline.departureTime) 
        newErrors[`departureTime-${index}`] = "Departure time is required";
      
      
      if (!airline.arrivalDate) newErrors[`arrivalDate-${index}`] = "Arrival date is required";
      if (!airline.arrivalTime) 
        newErrors[`arrivalTime-${index}`] = "Arrival time is required";
      
      // Date validation
      if (airline.departureDate && airline.arrivalDate) {
        if (airline.arrivalDate < airline.departureDate) {
          newErrors[`arrivalDate-${index}`] = "Arrival date cannot be before departure date";
        } else if (airline.arrivalDate === airline.departureDate && airline.arrivalTime <= airline.departureTime) {
          newErrors[`arrivalTime-${index}`] = "Arrival time must be after departure time for same-day flights";
        }
      }
      ...
      if (!airline.terminal?.trim()) newErrors[`terminal-${index}`] = "Terminal Name is required";
    });
      ...
      const response = await axios.post(
        `${API_BASE_URL}/staff/AddFlightDetails`,
        {
          ...
          airlines: bookingData.airlines.map(airline => ({
            ...
            departuretime: `${airline.departureDate}T${airline.departureTime}:00`,
            arrivaltime: `${airline.arrivalDate}T${airline.arrivalTime}:00`,
  ...
  if (confirmed) {
    ...
    try {
      ...
      const response = await submitFlightDetails(formData);
       // console.log(response.data)('API Response:', response); // Log the response
...
<div className="space-y-1">
  ...
  <div className="relative">
    <input
      type="date"
      name="departureDate"
      value={airline.departureDate}
      onChange={(e) => handleAirlineChange(index, e)}
      onClick={(e) => e.target.showPicker()} // This ensures the picker opens on any click
      className={`w-full px-4 py-2 rounded-lg border focus:ring-2 focus:outline-none transition-all ${
        errors[`departureDate-${index}`] 
          ? "border-red-500 focus:ring-red-300 dark:focus:ring-red-700" 
          : "border-gray-300 dark:border-gray-600 focus:border-green-600 focus:ring-green-200 dark:focus:ring-green-800"
      } bg-white dark:bg-gray-700 text-gray-900 dark:text-white`}
    />
    {errors[`departureDate-${index}`] && (
      <p className="text-red-500 text-sm mt-1">{errors[`departureDate-${index}`]}</p>
    )}
  </div>
</div>
...
<div className="space-y-1">
  ...
  <div className="relative">
    <input
      type="date"
      name="arrivalDate"
      value={airline.arrivalDate}
      onChange={(e) => handleAirlineChange(index, e)}
      onClick={(e) => e.target.showPicker()}  // Added this line
      min={airline.departureDate || undefined}
      className={`w-full px-4 py-2 rounded-lg border focus:ring-2 focus:outline-none transition-all ${
    ...
    />
    {errors[`arrivalDate-${index}`] && (
      <p className="text-red-500 text-sm mt-1">{errors[`arrivalDate-${index}`]}</p>
    )}
  </div>
</div>
...
<div className="space-y-1">
  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
    Arrival Time (HH:MM) <span className="text-red-500">*</span>
  </label>
  <input
    type="text"
    name="arrivalTime"
...
<div className="space-y-1">
  ...
  <input
    ...
    className={`w-full px-4 py-2 rounded-lg border focus:ring-2 focus:outline-none transition-all ${
      errors[`terminal-${index}`] 
        ? "border-red-500 focus:ring-red-300 dark:focus:ring-red-700" 
        : "border-gray-300 dark:border-gray-600 focus:border-green-600 focus:ring-green-200 dark:focus:ring-green-800"
    } bg-white dark:bg-gray-700 text-gray-900 dark:text-white`}
  />
  {errors[`terminal-${index}`] && (
    <p className="text-red-500 text-sm mt-1">{errors[`terminal-${index}`]}</p>
  )}
</div>
...
<p className="font-medium text-gray-900 dark:text-white">
  <span className="text-green-600">Passengers:</span> {formatPassengerNames(booking.passengerNames)}
</p>
            ...
            {booking.airlines && booking.airlines.map((airline, idx) => (
              <div key={idx} className="mt-2 border-t pt-2">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="text-green-600">Terminal:</span> {airline.terminal}
                </p>
              </div>
            ))}
          </div>
