...
const StaffManagement = () => {
  ...
  const validateEmpId = (empid) => {
    ...
    if (!/^SS\d+$/.test(empid)) return "Employee ID must start with SS followed by numbers";
  ...
  const openModal = (type, staff = null) => {
    ...
    if (type === "edit" && staff) {
      setFormData({
        ...
        password: "", // Always require password on edit
        empid: staff.empid,
        number: staff.number,
      });
    } else {
      setFormData({ 
        ...
        username: "", 
        password: "", 
        empid: "SS0", // Default prefix for new employees
        number: "" 
      });
    }
  ...
  const handleInputChange = (e) => {
    ...
    // For employee ID, ensure it starts with SS
    if (name === "empid" && modal.type === "add") {
      // If user tries to delete SS, prevent it
      if (value.length < 2) {
        setFormData({ ...formData, [name]: "SS" });
        return;
      }
      // Ensure first two characters are SS
      if (value.length === 2 && value !== "SS") {
        setFormData({ ...formData, [name]: "SS" + value.slice(2) });
        return;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        password: formData.password // Always send password
      };
  ...
  const handleDelete = async (staffId) => {
    try {
      ...
      closeDeleteModal();  // This will now properly close the modal
    } catch (err) {
            ...
            <div className="dark:bg-gray-800 bg-white rounded-lg shadow-xl w-full max-w-md p-6">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  <div className="relative">
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder={modal.type === "edit" ? "Enter current or new password" : "Create password"}
                      className={`w-full p-3 border rounded-lg text-gray-900 dark:text-white bg-white dark:bg-gray-900 ${
                  ...
                  {modal.type === "edit" && (
                    <p className="text-gray-500 text-xs mt-1">
                      You must enter either the current password or a new password
                    </p>
                  )}
                </div>
                ...
                <div className="mb-3">
                  ...
                  {modal.type === "add" && (
                    <p className="text-gray-500 text-xs mt-1">
                      Employee ID must start with SS followed by numbers (e.g., SS001)
                    </p>
                  )}
                </div>
